...
def collate_x_dict_simple(lst_x_dict: List, *, device: Optional[str] = None) -> Dict:
...
def collate_x_dict(lst_x_dict: List, *, device: Optional[str] = None) -> Dict:
    # if using simple "x" flattened features as input data
    if "x" in lst_x_dict[0].keys():
        return collate_x_dict_simple(lst_x_dict, device=device)

    # if using dict with key-value pairs as input data
    batch = {}
    for k in lst_x_dict[0].keys():
        if k == "length":
            length = [x_dict[k] for x_dict in lst_x_dict]
        else:
            batch[k] = collate_tensor_with_padding([x_dict[k] for x_dict in lst_x_dict])
    if device is not None:
        for k, v in batch.items():
            if k != "length":
                batch[k] = v.to(device)

    mask = length_to_mask(length, device=device)
    batch.update({"length": length, "mask": mask})
    return batch
