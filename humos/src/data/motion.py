...
class AMASSMotionLoader:
    def __init__(
            self, base_dir, fps, normalizer=None, disable: bool = False, canonicalize_crops: bool = False
    ):
        self.fps = fps
        ...
        if len(motion["trans"]) < self.num_frames:
            for k, v, in motion.items():
                ...
                if 'root_trans_diff' in k: # if velocity, then append zeros at the end
                    motion[k] = torch.cat((v, torch.zeros(self.num_frames - len(v), v.shape[1])) , dim=0)
                elif 'rot_orient_diff' in k: # if rot_orint_diff, append identity 6d pose at the end
                    ...
                    motion[k] = torch.cat((v, v[-1].repeat(self.num_frames - len(v), *([1] * (ndims - 1)))), dim=0)
    ...
    def load(self):
        ...
        for f in os.listdir(self.base_dir):
            if f.endswith("_mean.pt"):
                mean[f.replace("_mean.pt", "")] = torch.load(os.path.join(self.base_dir, f))
            if f.endswith("_std.pt"):
                std[f.replace("_std.pt", "")] = torch.load(os.path.join(self.base_dir, f))
        self.mean = mean
        self.std = std
