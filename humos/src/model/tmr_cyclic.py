...
class CYCLIC_TMR(TEMOS):
    ...
    def validation_step(self, batch: Dict, batch_idx: int) -> Tensor:
        ...
        smpl_params = smplh_breakdown(pred_dict_un, fk=self.fk_male)

        tag = "B_giv_A"

        save_path = os.path.join(out_dir, f"{keyids_A[0]}_{tag}.pt")
        torch.save(smpl_params, save_path)

        data_info = ",".join([f"{k}: {str(v.shape)}" for k, v in smpl_params.items()])

        print(f"[debug] saved: {save_path}. {data_info}")
