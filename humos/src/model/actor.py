...
class ACTORStyleEncoder(nn.Module):
    ...
    def __init__(
        ...
        latent_dim: int = 320,
        num_betas: int = 10,
        latent_dim_motion: int = 256,
        latent_dim_betas: int = 64,
        ff_size: int = 1024,
        num_layers: int = 4,
        num_heads: int = 4,
        dropout: float = 0.1,
        activation: str = "gelu",
        add_gender: bool = True,
    ) -> None:
        super().__init__()
        ...
        self.identity_projection = nn.Linear(num_betas, latent_dim_betas)

        self.projection = nn.Linear(nfeats, latent_dim_motion)
    ...
    def forward(self, x_dict: Dict) -> Tensor:
        x = x_dict["x"]
        mask = x_dict["mask"]
        identity = x_dict["identity"]


        # Append projected identity to the projected input
        x = self.projection(x)
...
class ACTORStyleDecoder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        output_feats = nfeats + 11  # 10 betas + 1 gender
        self.nfeats = nfeats
    ...
    def forward(self, z_dict: Dict) -> Tensor:
        z = z_dict["z"]
        mask = z_dict["mask"]
        identity = z_dict["identity"]

        # if identity has time dimension, remove it
        if len(identity.shape) > 2:
            identity = identity[:, 0, :]

        # Append projected identity to the latent vector
        identity = self.identity_projection(identity)
        z = torch.cat((z, identity), dim=-1)
