...
from humos.utils.cluster import execute_task_on_cluster
from humos.utils.default_hparams import hparams
from humos.utils.misc_utils import is_float
...
def get_grid_search_configs(config, excluded_keys=[]):
    ...
    for k, v in flattened_config_dict.items():
        if isinstance(v, list):
            if k in excluded_keys:
                flattened_config_dict[k] = ['+'.join([str(vi) for vi in v])]
            elif len(v) > 1:
                hyper_params += [k]
    ...
    for exp_id, exp in enumerate(experiments):
        for param in excluded_keys:
            exp[param] = [float(item) if is_float(item) else item for item in exp[param].strip().split('+')]
...
def run_grid_search_experiments(
    ...
    different_configs, hyperparams = get_grid_search_configs(
        cfg,
        excluded_keys=['TRAINING/TRAIN_FEATS'],
    )
