...
from humos.utils.rotation_conversions import euler_angles_to_matrix, quaternion_to_matrix, rotation_6d_to_matrix


class ForwardKinematicsLayer(nn.Module):
    """ Forward Kinematics Layer Class """

    def __init__(self, smpl_path=None, gender=None, parents=None, positions=None, num_joints=24, device=None):
        super().__init__()
        self.b_idxs = None
        if device is None:
        ...
        if parents is None and positions is None:
            ...
            smpl_fname = os.path.join(smpl_path, gender, 'model.npz')
        ...
        self.positions[0] = 0
        # filter joints to consider
        self.positions = self.positions[:num_joints, :]
