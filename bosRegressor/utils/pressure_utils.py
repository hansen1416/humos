...
class PressureMat:
    def __init__(self, csv_path, xml_path, c3d_file, get_pressure_map=True):
    ...
    def extract_pressure_mat_markers(self):
        ...
        marker_id_one = \
        np.where(data.point_labels == 'SensorMat:M_1                                                   ')[0][0]
        marker_id_two = \
        np.where(data.point_labels == 'SensorMat:M_2                                                   ')[0][0]
        marker_id_three = \
        np.where(data.point_labels == 'SensorMat:M_3                                                   ')[0][0]
        marker_id_four = \
        np.where(data.point_labels == 'SensorMat:M_4                                                   ')[0][0]
        ...
        marker_ids = [marker_id_bl, marker_id_tl, marker_id_tr,
                      marker_id_br]  # (bottom left, top left, top right, bottom right)
...
def pressure_to_contact_mask(vertices_hd, cop_w=10, cop_k=100, pressure_thresh=0.5):
    ...
    # v_y = inside_mask * np.exp(-4 * cop_w * vertex_height) + outside_mask * np.exp(-cop_w * vertex_height)
    pressure_weights = inside_mask * (1 - cop_k * vertex_height) + outside_mask * torch.exp(-cop_w * vertex_height)
    # normalize pressure weights to [0, 1]
    pressure_weights = (pressure_weights - torch.min(pressure_weights)) / (
            torch.max(pressure_weights) - torch.min(pressure_weights))
    pressure_mask = (pressure_weights > pressure_thresh)
    return pressure_mask, pressure_weights

def get_contact_mask(vertices_hd, contact_thresh=0.01, velocity_thresh=0.01):
    bs, nf, nv, _ = vertices_hd.shape

    ground_plane_height = 0.0
    vertex_height = (vertices_hd[:, :, :, 2] - ground_plane_height)
    # vertex_height_robustified = GMoF_unscaled(rho=gmof_rho)(vertex_height)

    # threshold on the distance from ground and velocity of the vertices
    vertex_vel = vertices_hd[:, 1:, :, :] - vertices_hd[:, :-1, :, :]
    vertex_vel = torch.norm(vertex_vel, dim=-1)
    # repeat last velocity row to match the number of vertices
    vertex_vel = torch.cat([vertex_vel, vertex_vel[:, -1:, :]], dim=1)
    contact_mask = torch.logical_and((vertex_height < contact_thresh), (vertex_vel < velocity_thresh))
    return contact_mask
